    ...
    showMyRecipe() {
        ...
        try {
            firebase.database().ref('/users/'+userid).on("value", function (snapshot) { 
                rates=snapshot.child('rates').val()
                firebase.database().ref().child('users/'+userid+'/follwing').on("value", function (snapshot) {
                 ...
                 // console.log('followingrates',rates.ar)
            for(let i=0;i<followingrates.length;i++)
            {
                 score=0
                 if(rates.ar===followingrates[i].ar)
                 { 
                      score++;
                 }
                 if(rates.eu===followingrates[i].eu)
                 { 
                      score++;
                 }
                 if(rates.as===followingrates[i].as)
                 { 
                      score++;
                 }
                 if(rates.fa ===followingrates[i].fa)
                 { 
                      score++;
                 }
